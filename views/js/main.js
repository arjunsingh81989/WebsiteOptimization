...
var resizePizzas = function(size) {
  ...
  // FIXED: return the size scale corresponding to value "1", "2", and "3".
  // This eliminates the calls to 'sizeSwitcher' for every pizza's elements
  ...
  function changePizzaSizes(size) {
    ...
    function determineDx (elem, newSize) {
    ...
    // FIXED: query all elements having class randomPizzaContainer, instead
    // of calling this per iteration
    var pizzaContainers = document.getElementsByClassName("randomPizzaContainer");
    
    // FIXED: call 'determineDx' once for the first element
    // (instead of per "pizza" element) because all pizzas have the same size
    var dx = determineDx(pizzaContainers[0], size);
    var newwidth = (pizzaContainers[0].offsetWidth + dx) + 'px';
    // Now loop over all "pizza" elements and set the same new width
    // (instead of calculating new width per element)
    for (var i = 0; i < pizzaContainers.length; i++) {
      pizzaContainers[i].style.width = newwidth;
    }
  }

  // FIXED: changePizzaSizes now takes the new size scale, 
  // instead of "1", "2", or "3"
  changePizzaSizes(changeSliderLabel(size));
...
function updatePositions() {
  ...
  // FIXED: pre-calculate the five phases instead of calculating phase
  // per item.
  var phaseAngle = document.body.scrollTop / 1250;
  var phases = [
      100 * Math.sin(phaseAngle), 
      100 * Math.sin(phaseAngle + 1), 
      100 * Math.sin(phaseAngle + 2),
      100 * Math.sin(phaseAngle + 3), 
      100 * Math.sin(phaseAngle + 4)
  ];
  // FIXED: Use array of references to 'mover' elements, instead of query them
  // every scroll
  for (var i = 0; i < moverPizzas.length; i++) {
    var phase = phases[i % 5];
    moverPizzas[i].style.left = moverPizzas[i].basicLeft + phase + 'px';
  }
...
// FIXED: create an array that has a reference to all of the 'mover' pizzas
var moverPizzas = [];
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // FIXED: About 60 pizzas can fill the whole screen
  for (var i = 0; i < 60; i++) {
    ...
    moverPizzas.push(elem); // FIXED: save reference to the pizza element
  }
  updatePositions();
});
